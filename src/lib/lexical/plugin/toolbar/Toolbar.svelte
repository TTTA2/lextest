<script lang="ts">
    ...
    let { editor, h1, onChange, registerToolbar = $bindable() }: 
    {
        editor: LexicalEditor, 
        h1?: boolean, 
        onChange?: (formattedStyle: FormattedStyle) => void,
        registerToolbar?: ((editor: LexicalEditor, onUpdateFormat: (style: FormattedStyle) => void) => void) | undefined,

    } = $props();

    let formattedStyle: FormattedStyle = $state({
        head: undefined,
        bold: false,
        italic: false,
        strikethrough: false,
    ...
    function handleChange(newFormattedStyle: FormattedStyle, style?: Style | undefined) {

        console.log(newFormattedStyle);
...
<div on:mousedown={(e) => e.preventDefault()}>
    ...
    <button on:click={(e) => handleChange({ ...formattedStyle, head: formattedStyle.head == "h1" ? undefined : "h1" }) }>H1</button>
    <button on:click={(e) => handleChange({ ...formattedStyle, head: formattedStyle.head == "h2" ? undefined : "h2" }) }>H2</button>
    <button on:click={(e) => handleChange({ ...formattedStyle, head: formattedStyle.head == "h3" ? undefined : "h3" }) }>H3</button>

    <button on:click={(e) => handleChange({ ...formattedStyle, bold: !formattedStyle.bold })}>BOLD</button>
    <button on:click={(e) => handleChange({ ...formattedStyle, italic: !formattedStyle.italic })}>ITALIC</button>
    <button on:click={(e) => handleChange({ ...formattedStyle, strikethrough: !formattedStyle.strikethrough })}>STRIKETHROUGH</button>
